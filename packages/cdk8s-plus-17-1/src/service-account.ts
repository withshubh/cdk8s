...
import * as k8s from './imports/v17/k8s';
...
export class ServiceAccount extends Resource implements IServiceAccount {
  ...
  private _specProvider: () => k8s.ServiceAccountOptions;

  constructor(scope: Construct, id: string, props: ServiceAccountProps = { }) {
    super(scope, id, { metadata: props.metadata });

    this._secrets = props.secrets ?? [];
    this._specProvider = this._spec;

    this.apiObject = new k8s.ServiceAccount(this, 'Resource', {
      metadata: props.metadata,
      secrets: Lazy.any({ produce: () => this._specProvider().secrets }),
      automountServiceAccountToken: Lazy.any({ produce: () => this._specProvider().automountServiceAccountToken }),
      imagePullSecrets: Lazy.any({ produce: () => this._specProvider().imagePullSecrets }),
    });
  }
   ...
   * @internal
   */
  public _setSpecProvider(specProvider: () => k8s.ServiceAccountOptions) {
   ...
   * @internal
   */
  public _spec(): k8s.ServiceAccountOptions {
    return {
      secrets: undefinedIfEmpty(this._secrets.map(s => ({ name: s.name }))),
    };
  }
}